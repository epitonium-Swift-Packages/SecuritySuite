...
internal class DebuggerChecker {
  ...
  static func amIDebugged() -> Bool {
    ...
    if sysctlRet != 0 {
      Log?.warning(mesage: "SecuritySuite: Error occured when calling sysctl(). The debugger check may not be reliable")
    }
  ...
  static func denyDebugger() {
    ...
    if ptraceRet != 0 {
      Log?.warning(mesage: "SecuritySuite: Error occured when calling ptrace(). Denying debugger may not be reliable")
    }
  }
